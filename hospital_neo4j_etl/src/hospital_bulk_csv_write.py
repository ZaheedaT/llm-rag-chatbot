# Neo4j's query language is called Cypher. "MATCH" = used to specify patterns or things to look for in the graph
# Nodes are indicated by parentheses
# Real power of Cypher comes from its ability to match on relationship patterns.
# Relationships are indicated by square brackets [h:HAS]-> ARROW direction is important
import logging
import os
...
# Decorator if load_hospital_graph_from_csv() fails, the decorator will rerun it 100 times with a 10 sec delay between
@retry(tries=100, delay=10)
def load_hospital_graph_from_csv() -> None:
    """Load structured hospital CSV data following
    a specific ontology into Neo4j

    GraphDatabase: connects app to running instance"""
